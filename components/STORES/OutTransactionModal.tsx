...
const OutTransactionModal: React.FC<Props> = ({ isOpen, onClose, title, transactions, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 flex flex-col max-h-[95vh] w-full max-w-md">
        ...
        <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-rose-500 bg-opacity-10">
          <h2 className="font-bold text-sm text-rose-500">OUT - {title}</h2>
          ...
          {localTrans.length === 0 ? ( <div className="text-center text-slate-400 text-xs py-10">No records added yet.</div> ) : (
            <table className="w-full text-left text-[11px] whitespace-nowrap">
              <thead className="text-[9px] uppercase font-bold text-slate-500 sticky top-0 bg-slate-100">
                <tr><th className="px-3 py-2">Date</th><th className="px-3 py-2 text-right">Value (OUT)</th><th className="px-3 py-2 text-center">Act</th></tr>
              </thead>
              <tbody>
                {localTrans.map((t, i) => (
...
export default OutTransactionModal;