...
import { Search, Filter, X, ChevronDown, Check, Layers, Download } from 'lucide-react';
import MultiSelectDropdown from './MultiSelectDropdown';
import { exportToExcel, getDatesInRange } from '../utils';
...
const ProductionTab: React.FC<Props> = ({ date: initialDate, allData, onUpdate, adminConfig }) => {
   ...
   // Extract available machines and products for filters
   const { availableMachines, availableProducts } = React.useMemo(() => {
      const machines = new Set<string>();
      const products = new Set<string>();

      Object.keys(allData).forEach(key => {
         // Only data matching current machine type (IM/BM)
         if (key.includes(`_${machineType}`)) {
            allData[key].rows.forEach((r: ProductionRow) => {
      ...
      return {
         availableMachines: Array.from(machines).sort(),
   ...
   const handleDownloadReport = async () => {
      ...
      const rowsToExport: (ProductionRow & { date: string })[] = [];

      dates.forEach(date => {
         const key = `${date}_${machineType}`;
         const dayDataForDate = allData[key];
         if (dayDataForDate && dayDataForDate.rows) {
            dayDataForDate.rows.forEach((row: ProductionRow) => {
               // Apply filters
               const machineMatch = selectedMachines.length === 0 || selectedMachines.includes(row.machine);
               const productMatch = selectedProducts.length === 0 || selectedProducts.includes(row.product);

               if (machineMatch && productMatch) {
                  rowsToExport.push({ ...row, date });
               }
            });
         }
      });
   ...
   return (
      <div className="flex flex-col h-full bg-[#F8FAFC] dark:bg-[#020617] transition-colors duration-300">
         ...
         {/* Main Content with MORE bottom padding to fix scroll issue */}
         <div className="flex-1 overflow-auto p-6 md:p-8 custom-scrollbar pb-64 bg-[#F8FAFC] dark:bg-[#020617] transition-colors duration-300">
            ...
            {(subTab === 'IM_DB' || subTab === 'BM_DB') && (
               <div className="animate-fade-in">
                  <div className="mb-8 flex items-end justify-between">
                     ...
                     <div className="flex items-center gap-3">
                        ...
                        <button
                           ...
                           className="h-11 px-5 flex items-center gap-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl transition-all shadow-lg shadow-indigo-600/20 font-black uppercase text-[10px] tracking-widest"
                        ...
                        {(selectedMachines.length > 0 || selectedProducts.length > 0) && (
                           <button
                              onClick={clearFilters}
                              className="h-11 w-11 flex items-center justify-center bg-rose-50 dark:bg-rose-900/20 text-rose-500 rounded-xl hover:bg-rose-100 dark:hover:bg-rose-900/40 transition-colors border border-rose-100 dark:border-rose-900/50"
                              title="Clear Filters"
                           >
                              <X className="w-5 h-5" />
                           </button>
                        ...
                        {/* DATE RANGE */}
                        <div className="flex items-center bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-xl p-1 shadow-sm h-11">
                           <div className="px-3 flex items-center gap-2 border-r border-slate-200 dark:border-slate-600">
                              <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest hidden lg:block">FROM</span>
                           ...
                           <div className="px-3 flex items-center gap-2">
                              <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest hidden lg:block">TO</span>
                  ...
                  <DatabaseView
                     ...
                     allData={allData}
                     machineFilter={selectedMachines}
            ...
            {subTab === 'SUMMARY' && <DailySummary allData={allData} date={entryDate} breakdownCategories={adminConfig.breakdownCategories || []} />}
         </div>

         {
            activeRowForBD && (
               <BreakdownModal
                  ...
                  categories={adminConfig.breakdownCategories}
               />
            )
         }
...
const CustomDropdown: React.FC<{ options: string[], value: string, onChange: (val: string) => void, label: string, icon?: React.ReactNode }> = ({ options, value, onChange, label, icon }) => {
   ...
   return (
      <div className="relative" ref={dropdownRef}>
         <button
            ...
            className={`h-11 px-4 rounded-xl flex items-center gap-3 border transition-all ${isOpen ? 'bg-indigo-50 border-indigo-500 dark:bg-indigo-900/20 dark:border-indigo-500' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:border-indigo-300 dark:hover:border-indigo-700'}`}
         >
         ...
         {isOpen && (
            <div className="absolute top-full left-0 mt-2 w-56 bg-white dark:bg-slate-800 rounded-xl shadow-xl shadow-slate-200/50 dark:shadow-black/50 border border-slate-100 dark:border-slate-700 overflow-hidden z-[60] animate-in fade-in zoom-in-95 duration-100">
               <div className="max-h-64 overflow-y-auto custom-scrollbar p-1.5">
                  <div
                     onClick={() => { onChange(''); setIsOpen(false); }}
                     className={`px-3 py-2.5 rounded-lg cursor-pointer flex items-center justify-between text-xs font-bold transition-colors ${value === '' ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50'}`}
                  >
                  ...
                  {options.map(opt => (
                     <div
                        ...
                        className={`px-3 py-2.5 rounded-lg cursor-pointer flex items-center justify-between text-xs font-bold transition-colors ${value === opt ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50'}`}
                     >
                        <span>{opt}</span>
